...
let menuElemanlari = [
 ...
 /* <div class="menu">
  ...
  buna  bir `click` event listener ekleyin. Butona her tıklanıldığında div.menu classına('menu' class'ına sahip olan div) 
  `menu--open` ekleyip/silecek(toggle).*/

function MenuYapici(menuItems) {
    const menu = document.createElement("div");
    menu.classList.add("menu");

    const madde = document.createElement("ul");

    menuItems.forEach(i => {
        const eleman = document.createElement("li");
        eleman.textContent = i;
        madde.appendChild(eleman);
    })

    menu.appendChild(madde);
...
/*  Adım 4: 'MenuYapici' fonksiyonunda oluştrulan div.menu'yü döndürmeyi unutmayın.
  ...
  Adım 5: 'MenuYapici' fonksiyonunu ve 'menuElemanlari' dizisini kullanarak menüyü oluşturun, 
  ve döndürülen menüyü header class'ına sahip div'e ekleyin.
  */
