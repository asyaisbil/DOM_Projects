...
const data = [
  ...
  data.forEach((item) => {
    document.querySelector('div.articles').appendChild(HaberYapici(item));
  });
  
  /* Adım 4: Fonksiyonunuzun dışında, tüm haberleri içeren datayı döngüye sokun(loop). 
  Yazdığınız 'HaberYapici' component'ini kullanarak bir div.article öğesi oluşturun ve 
  bunu DOM'daki div.articles düğümüne ekleyin. (bknz. index.html).
